...
def main():
    ...
    total_val = len(original_data.get("questions", []))
    progress_data = {"completed": 0, "total": total_val, "issues": []}
    ...
    completed_in_dataset = sum(1 for q in original_data.get("questions", []) if "subagent" in str(q.get("explanation_model", "")).lower())
    progress_data["completed"] = completed_in_dataset
    progress_data["completed_questions"] = completed_in_dataset
    progress_data["total"] = progress_data.get("total") or progress_data.get("total_questions") or total_val
    progress_data["total_questions"] = progress_data["total"]
    
    # Append new issues, avoid duplicates
    if "issues" not in progress_data:
        progress_data["issues"] = []
    existing_issues = set(progress_data["issues"])
    for iss in issues:
